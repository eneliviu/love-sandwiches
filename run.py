...
def validate_data(values):
    ...
    try:
        ...
        if len(values) != 6:
            raise ValueError(
                    f'Exactly 6 vakues required, you provided {len(values)}'
                )
...
def update_worksheet(data, worksheet):
    ...
    worksheet_data.append_row(data)
...
def get_last_5_entries_sales():
    '''
    Collects collumns of data fro sales worksheet, collecting 
...
def get_stock_values(data):
    ...
    print(headings)
    stoc_dict = {str:num for (str, num) in zip(headings, data)}
    return stoc_dict
...
print('Welcome to Love Sandwiches Data Automation modified!')
stock_data = main()
